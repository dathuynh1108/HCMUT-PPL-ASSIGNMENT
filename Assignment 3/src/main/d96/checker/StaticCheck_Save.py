...
class D96_utils:
    ...
    def coercion(type_1, type_2, inheritance):  # Check a type can coercion to another type
        ...
        if type(type_2) == ClassType and type(type_1) == ClassType:
            ...
            while parent_name:
                if type_2.classname.name == parent_name:
                    return True
    ...
    def find_attribute(class_name, attribute_name, global_scope, inheritance):
        ...
        while current_class:
            if attribute_name in global_scope[current_class]:
                return global_scope[current_class][attribute_name]
...
class StaticChecker(BaseVisitor):
    ...
    def visitClassDecl(self, ast, scope):
        ...
        for mem in ast.memlist:
            ...
            if ast.classname.name == "Program" and isinstance(mem, MethodDecl) and mem.name.name == "main" and mem.param == []:
                entry_point_fail = False
    ...
    def visitVarDecl(self, ast, scope):
        ...
        if "local" not in scope:  # Attribute
            ...
            if ast.variable.name in scope["global"][self.current_class]:
                raise Redeclared(Attribute(), ast.variable.name)
            scope["global"][self.current_class][ast.variable.name] = D96_type(
                "mutable", kind, decl_type)
        ...
        # --> push to local and check redeclare (Class scope has been checked)
            ...
            scope["local"][0][ast.variable.name] = D96_type(
                "variable", None, decl_type)

        if ast.varInit:
            if isinstance(init_type, D96_type):
                init_type = init_type.type
            if not D96_utils.compare(init_type, decl_type) and not D96_utils.coercion(init_type, decl_type, self.inheritance):
    ...
    def visitConstDecl(self, ast, scope):
        ...
        init_type = self.visit(
            ast.value, (ast.value, scope)) if ast.value else None
        if "local" not in scope:  # attribute
            kind = "instance" if isinstance(kind, Instance) else "static"
            if ast.constant.name in scope["global"][self.current_class]:
                raise Redeclared(Attribute(), ast.constant.name)
            scope["global"][self.current_class][ast.constant.name] = D96_type(
                "imutable", kind, decl_type)
        ...
        if "local" in scope:  # if not in class context because class context has been check
            ...
            scope["local"][0][ast.constant.name] = D96_type(
                "constant", None, decl_type)
    ...
    def visitMethodDecl(self, ast, scope):
        ...
        scope["global"][self.current_class][ast.name.name] = D96_type(
            "method", si_kind, None, param_type)
    ...
    def visitNewExpr(self, ast, scope):
        ...
        if len(ast.param) != 0:
            if "Constructor" in scope["global"][ast.classname.name]:
                ...
                argument_type = [self.visit(param, scope)
                                 for param in ast.param]
                if not D96_utils.check_param_type(class_constructor, argument_type, self.inheritance):
                    raise TypeMismatchInExpression(ast)
            else:
    ...
    def visitFieldAccess(self, ast, scope):
        ...
        if type(ast.obj) == SelfLiteral:
            ...
            if self.current_method and scope["global"][self.current_class][self.current_method].si_kind == "static":
                raise IllegalMemberAccess(ast)
            # Self --> find in current class, and need to be instance
            field_name_type = D96_utils.find_attribute(
                self.current_class, ast.fieldname.name, scope["global"], self.inheritance)
            ...
            if field_name_type.si_kind == "static":
                raise IllegalMemberAccess(ast)
            if const_expresion and field_name_type.kind == "mutable":
                raise IllegalConstantExpression(const_expresion)
        ...
        if isinstance(ast.obj, Id):
            ...
            if "$" in ast.fieldname.name:  # A class name or a attribute
                ...
                if field_name_type is None:
                    raise Undeclared(Attribute(), ast.fieldname.name)
                if field_name_type.kind == "method":
                    Undeclared(Attribute(), ast.fieldname.name)
            ...
            if obj_type:  # is a local
                ...
                if type(obj_type) != ClassType:
                    raise TypeMismatchInExpression(ast)
                field_name_type = D96_utils.find_attribute(
                    obj_type.classname.name, ast.fieldname.name, scope["global"], self.inheritance)
                if field_name_type is None:
                    raise Undeclared(Attribute(), ast.fieldname.name)
                if field_name_type.kind == "method":
                    raise Undeclared(Attribute(), ast.fieldname.name)
    ...
    def visitCallExpr(self, ast, scope):
        ...
        if type(ast.obj) == SelfLiteral:
            if self.current_method and scope["global"][self.current_class][self.current_method].si_kind == "static":
                raise IllegalMemberAccess(ast)
            call_method = D96_utils.find_attribute(
    ...
    def visitId(self, ast, scope):  # Only use to find local variable
        ...
        if "local" in scope:
            ...
            if local_id:
                if const_expression and (local_id.kind == "mutable" or local_id.kind == "variable"):
                    raise IllegalConstantExpression(const_expression)
                return local_id
    ...
    def visitArrayLiteral(self, ast, scope):
        type_of_element_list = [self.visit(
            value, scope) for value in ast.value]
    ...
    def visitAssign(self, ast, scope):
        ...
        if isinstance(lhs_type, D96_type):
            if lhs_type.kind == "imutable" or lhs_type.kind == "constant":
    ...
    def visitFor(self, ast, scope):
        ...
        if id_type.kind == "constant" or id_type.kind == "imutable":
    ...
    def visitReturn(self, ast, scope):
        ...
        if self.current_method == "main" and self.current_class == "Program" and ast.expr is not None:
            raise TypeMismatchInStatement(ast)
        if self.current_method == "Destructor":
            raise TypeMismatchInStatement(ast)
        currnet_return_type = self.visit(
            ast.expr, scope) if ast.expr else VoidType()
