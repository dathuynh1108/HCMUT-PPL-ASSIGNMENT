...
def print_scope(scope):
    ...
    print("Parent class:", scope["parent"])
    print("Local: [")
    if "local" in scope:
...
class Get_global_scope(BaseVisitor, Utils): # Get all class, their attribuite and inheritance list
    def __init__(self):
        # first member is global scope of all class
        # second member is inheritance list of all class
        # Only check to class scope, not check local scope and type
        self.global_scope = ({}, {})
        #self.has_entry_point = False
    
    def get_all_class_scope(self, ast):
    ...
    def visitClassDecl(self, ast: ClassDecl, scope):
        if ast.classname.name in scope[0]: raise Redeclared(Class(), ast.classname.name)
        scope[0][ast.classname.name] = {}
        if ast.parentname:
            if ast.parentname.name not in scope[0]: raise Undeclared(Class(), ast.parentname.name)
            scope[1][ast.classname.name] = ast.parentname.name
        else:
            scope[1][ast.classname.name] = None
        #program_flag = False
        #if ast.classname.name == "Program": program_flag = True
        for mem in ast.memlist: self.visit(mem, scope[0][ast.classname.name])
    ...
    def visitVarDecl(self, ast: VarDecl, scope): 
        si_kind, scope = scope[0], scope[1]
        si_kind = "instance" if isinstance(si_kind, Instance) else "static"
        if ast.variable.name in scope: raise Redeclared(Attribute(), ast.variable.name)
        scope[ast.variable.name] = D96_type("mutable", si_kind, self.visit(ast.varType, scope))
    
    def visitConstDecl(self, ast: ConstDecl, scope): 
        si_kind, scope = scope[0], scope[1]
        si_kind = "instance" if isinstance(si_kind, Instance) else "static"
        if ast.constant.name in scope: raise Redeclared(Attribute(), ast.constant.name)
        scope[ast.constant.name] = D96_type("imutable", si_kind, self.visit(ast.constType, scope))

    def visitMethodDecl(self, ast, scope):
        if ast.name.name in scope: raise Redeclared(Method(), ast.name.name)
        ...
        scope[ast.name.name] = D96_type("method", si_kind, None, param_type)
    ...
    def visitClassType(self, ast, scope):
        if ast.classname.name in self.global_scope[0]:
            return ast
...
class StaticChecker(BaseVisitor,Utils):    
    def __init__(self,ast):
        self.ast = ast
        self.global_scope = {} # Global scope, include all class and their attr, method
    ...
    def visitClassDecl(self, ast, scope):
        scope["current"] = ast.classname.name
        # if ast.parentname:
        #     if ast.parentname.name not in scope["global"]: raise Undeclared(Class(), ast.parentname.name)
        #     scope["parent"] = ast.parentname.name
        # else: scope["parent"] = None
        # scope["parent"] = ast.parentname.name if ast.parentname else None
        #print_scope(scope)
        for mem in ast.memlist: self.visit(mem, scope)

    def visitAttributeDecl(self, ast, scope):
        kind = Variable() if isinstance(ast.decl, VarDecl) else Constant()
        self.visit(ast.decl, (kind, scope))
    
    def visitVarDecl(self, ast, scope):
        # Check redeclare:
        kind, scope = scope # Break in to multi kind to use with param type
        decl_type = self.visit(ast.varType, scope)
        if "local" not in scope: # Attribute
            # Prevent Var a = Self.a or a
            # Because all attribute is push in Get_all_class --> Self.a or a on RHS is found in class scope
            # ---> need to pop current declare attribute --> RHS is not found
            # ---> after check init --> push back
            save = scope["global"][scope["current"]][ast.variable.name]
            scope["global"][scope["current"]].pop(ast.variable.name)
            init_type = self.visit(ast.varInit, scope) if ast.varInit else None
            scope["global"][scope["current"]][ast.variable.name] = save
        else: init_type = self.visit(ast.varInit, scope) if ast.varInit else None
        # if not in class scope (in local)
        # --> push to local and check redeclare (Class scope has been checked)
        if "local" in scope: # Local scope
    ...
    def visitConstDecl(self, ast, scope):
        ...
        if "local" not in scope:  # attribute
            ...
            # ---> need to pop current declare attribute --> RHS is not found
            # ---> after check init --> push back
            save = scope["global"][scope["current"]][ast.constant.name]
            scope["global"][scope["current"]].pop(ast.constant.name)
            init_type = self.visit(ast.value,  (True, scope))
            scope["global"][scope["current"]][ast.constant.name] = save
        else: init_type = self.visit(ast.value, (True, scope))
        # Check redeclare:
