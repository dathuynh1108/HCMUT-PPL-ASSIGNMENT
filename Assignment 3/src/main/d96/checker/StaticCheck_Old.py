...
class D96_utils:
    ...
    def check_param_type(method, argument_type, inheritance):
        ...
        for i in range(len(argument_type)): 
            current_argument_type = argument_type[i].type if isinstance(argument_type[i], D96_type) else argument_type[i]
            if not D96_utils.compare(current_argument_type, method.param_type[i]) and not D96_utils.coercion(current_argument_type, method.param_type[i], inheritance): return False
        return True
