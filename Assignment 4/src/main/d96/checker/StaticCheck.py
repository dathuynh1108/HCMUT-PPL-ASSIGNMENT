...
class D96_type:
    ...
    def __str__(self):
        return (
            ...
            + " "
            + str(self.type)
            + (" " + str(self.param_type) if self.param_type is not None else "")
            + "]"
        )
...
class D96_utils:
    ...
    def compare(type_1, type_2):  # Compare 2 type
        ...
        if isinstance(type_1, D96_type):
            type_1 = type_1.type
        if isinstance(type_2, D96_type):
            type_2 = type_2.type
        if type(type_1) == ArrayType and type(type_2) == ArrayType:
            return type_1.size == type_2.size and D96_utils.compare(
                type_1.eleType, type_2.eleType
            )
        # Same class type
    ...
    @staticmethod
    def coercion(
        type_1, type_2, inheritance
    ):  # Check a type can coercion to another type
        if isinstance(type_1, D96_type):
            type_1 = type_1.type
        if isinstance(type_2, D96_type):
            type_2 = type_2.type
        if type(type_2) == FloatType and type(type_1) == IntType:
            return True
        # Check type_2 is parent type_1
        # print(type_1, type_2)
        if type(type_2) == ClassType and type(type_1) == NullLiteral:
            return True
        ...
        if type(type_2) == ArrayType and type(type_1) == ArrayType:
            return type_2.size == type_1.size and D96_utils.coercion(
                type_1.eleType, type_2.eleType, inheritance
            )
        return False
    ...
    def check_param_type(method, argument_type, inheritance):
        if len(method.param_type) != len(argument_type):
            return False
        for i in range(len(argument_type)):
            if isinstance(argument_type[i], D96_type):
                argument_type[i] = argument_type[i].type
            if not D96_utils.compare(
                argument_type[i], method.param_type[i]
            ) and not D96_utils.coercion(
                argument_type[i], method.param_type[i], inheritance
            ):
                return False
        return True
...
class StaticChecker(BaseVisitor):
    ...
    def visitProgram(self, ast, scope):
        ...
        for decl in ast.decl:
            ...
            if decl.classname.name == "Program":
                has_program_class = True
        if not has_program_class:
            raise NoEntryPoint()
        return []
    ...
    def visitClassDecl(self, ast, scope):
        ...
        if ast.classname.name == "Program" and not has_main_method:
            raise NoEntryPoint()
        self.current_class = None
    ...
    def visitVarDecl(self, ast, scope):
        ...
        if "local" not in scope:  # Attribute
            ...
            if scope["global"][self.current_class].find_attribute(ast.variable.name):
                raise Redeclared(Attribute(), ast.variable.name)
            scope["global"][self.current_class].insert_attribute(
                ast.variable.name, D96_type("mutable", kind, decl_type)
            )
        # if not in class scope (in local)
        # --> push to local and check redeclare (Class scope has been checked)
        else:  # Local scope --> Variable
            if ast.variable.name in scope["local"][0]:
                raise Redeclared(kind, ast.variable.name)
        ...
        if ast.varInit:
            ...
            if not D96_utils.compare(init_type, decl_type) and not D96_utils.coercion(
                init_type, decl_type, self.inheritance
            ):
    ...
    def visitConstDecl(self, ast, scope):
        ...
        if "local" not in scope:  # attribute
            kind = "instance" if isinstance(kind, Instance) else "static"
            if scope["global"][self.current_class].find_attribute(ast.constant.name):
                raise Redeclared(Attribute(), ast.constant.name)
            scope["global"][self.current_class].insert_attribute(
                ast.constant.name, D96_type("immutable", kind, decl_type)
            )

        if (
            "local" in scope
        ):  # if not in class context because class context has been check
            if ast.constant.name in scope["local"][0]:
                raise Redeclared(kind, ast.constant.name)
            scope["local"][0][ast.constant.name] = D96_type("constant", None, decl_type)

        # Check type
        if ast.value:
            if isinstance(init_type, D96_type):
                if init_type.kind != "immutable" and init_type.kind != "constant":
                    raise IllegalConstantExpression(ast.value)
                init_type = init_type.type
            if not D96_utils.compare(init_type, decl_type) and not D96_utils.coercion(
                init_type, decl_type, self.inheritance
    ...
    def visitMethodDecl(self, ast, scope):
        # Check redeclare
        if scope["global"][self.current_class].find_method(ast.name.name):
        ...
        scope["global"][self.current_class].insert_method(
            ast.name.name, D96_type("method", si_kind, None, param_type)
        )
        ...
        new_scope = (
            scope.copy()
        )  # create new --> reference global and current --> add local
        new_scope["local"] = [{}]
        in_loop = False
        # Check param redeclare
        for param in ast.param:
            self.visit(param, (Parameter(), new_scope))
        # Check body
        for stmt in ast.body.inst:
            if isinstance(stmt, VarDecl):
                self.visit(stmt, (Variable(), new_scope))
            elif isinstance(stmt, ConstDecl):
                self.visit(stmt, (Constant(), new_scope))
            else:
                self.visit(stmt, (in_loop, new_scope))
        # Check and for no return type:
        if scope["global"][self.current_class].find_method(ast.name.name).type is None:
            scope["global"][self.current_class].find_method(
                ast.name.name
            ).type = VoidType()
        self.current_method = None
    ...
    def visitUnaryOp(self, ast, scope):
        ...
        if isinstance(body_type, D96_type):
            if body_type.kind != "immutable" and body_type.kind != "constant":
                expression_kind = "mutable"
            body_type = body_type.type
        if ast.op == "-":
            if type(body_type) == IntType or type(body_type) == FloatType:
                return D96_type(expression_kind, None, body_type)
            raise TypeMismatchInExpression(ast)
        if ast.op == "!" and type(body_type) == BoolType:
            return D96_type(expression_kind, None, body_type)
        raise TypeMismatchInExpression(ast)
    ...
    def visitBinaryOp(self, ast, scope):
        ...
        if isinstance(left_type, D96_type):
            if left_type.kind != "immutable" and left_type.kind != "constant":
                expression_kind = "mutable"
            left_type = left_type.type
        if isinstance(right_type, D96_type):
            if right_type.kind != "immutable" and right_type.kind != "constant":
                expression_kind = "mutable"
        ...
        if ast.op in ["+", "-", "*", "/"]:
            if not (
                ...
                and (type(right_type) == IntType or type(right_type) == FloatType)
            ):
                raise TypeMismatchInExpression(ast)
            if type(left_type) == FloatType or type(right_type) == FloatType:
                return D96_type(expression_kind, None, FloatType())
            return (
                D96_type(expression_kind, None, FloatType())
                if ast.op == "/"
                else D96_type(expression_kind, None, IntType())
            )

        if ast.op == "%":
            if type(left_type) == IntType and type(right_type) == IntType:
                return D96_type(expression_kind, None, left_type)
            raise TypeMismatchInExpression(ast)
        ...
        if ast.op == "==.":
            if type(left_type) == StringType and type(right_type) == StringType:
        ...
        if ast.op in ["==", "!="]:
            if D96_utils.compare(left_type, right_type):
                if type(left_type) == BoolType or type(left_type) == IntType:
                    return D96_type(expression_kind, None, BoolType())
            raise TypeMismatchInExpression(ast)
    ...
    def visitNewExpr(self, ast, scope):
        ...
        class_constructor = scope["global"][ast.classname.name].find_method(
            "Constructor"
        )
        if class_constructor:
            argument_type = [self.visit(param, scope) for param in ast.param]
            if not D96_utils.check_param_type(
                class_constructor, argument_type, self.inheritance
            ):
                raise TypeMismatchInExpression(ast)
        elif len(ast.param) != 0:
            raise TypeMismatchInExpression(ast)
    ...
    def visitArrayCell(self, ast, scope):
        ...
        for index_type in index_type_list:
            if type(index_type) == D96_type:
                index_type = index_type.type
            if type(array_type) != ArrayType:
                raise TypeMismatchInExpression(ast)
    ...
    def visitFieldAccess(self, ast, scope):
        if type(ast.obj) == SelfLiteral:
            ...
            if (
                self.current_method
                and self.current_method != "main"
                and scope["global"][self.current_class]
                .find_method(self.current_method)
                .si_kind
                == "static"
            ):
                raise IllegalMemberAccess(ast)
            # Self --> find in current class, and need to be instance
            field_name_type = D96_utils.find_attribute(
                self.current_class,
                ast.fieldname.name,
                scope["global"],
                self.inheritance,
            )
            if field_name_type is None:
                raise Undeclared(Attribute(), ast.fieldname.name)
            if field_name_type.kind == "method":
                raise Undeclared(Attribute(), ast.fieldname.name)
            if field_name_type.si_kind == "static":
                raise IllegalMemberAccess(ast)
        ...
        if isinstance(ast.obj, Id):
            ...
            if "$" in ast.fieldname.name:  # A class name or a attribute
                ...
                if ast.obj.name not in scope["global"]:
                    ...
                    if D96_utils.find_local(ast.obj.name, scope["local"]):
                        raise IllegalMemberAccess(ast)
                    # If not a variable --> Undeclare class
                    raise Undeclared(Class(), ast.obj.name)
                # If a class --> find attribute
                field_name_type = D96_utils.find_attribute(
                    ast.obj.name, ast.fieldname.name, scope["global"], self.inheritance
                )
                if field_name_type is None:
                    raise Undeclared(Attribute(), ast.fieldname.name)
                if field_name_type.kind == "method":
                    raise Undeclared(Attribute(), ast.fieldname.name)
                if field_name_type.si_kind == "instance":
                    raise IllegalMemberAccess(ast)
                # if const_expresion and field_name_type.kind == "mutable": raise IllegalConstantExpression(const_expresion)
                return field_name_type
            ...
            if ast.obj.name in scope["global"]:
                raise IllegalMemberAccess(ast)
        ...
        if field_name_type.si_kind == "static":
            raise IllegalMemberAccess(ast)
        # if const_expresion and field_name_type.kind == "mutable": raise IllegalConstantExpression(const_expresion)
        if field_name_type.kind != "immutable":
            expression_kind = "mutable"
    ...
    def visitCallExpr(self, ast, scope):
        ...
        if type(ast.obj) == SelfLiteral:
            if (
                ...
                raise IllegalMemberAccess(ast)
            call_method = D96_utils.find_method(
                self.current_class, ast.method.name, scope["global"], self.inheritance
            )
            if call_method is None:
                raise Undeclared(Method(), ast.method.name)
            if call_method.kind != "method":
                raise Undeclared(Method(), ast.method.name)
            if call_method.si_kind == "static":
                raise IllegalMemberAccess(ast)
            if type(call_method.type) == VoidType:
                raise TypeMismatchInExpression(ast)
            # Check type param
            argument_type = [self.visit(param, scope) for param in ast.param]
            if not D96_utils.check_param_type(
                call_method, argument_type, self.inheritance
            ):
                raise TypeMismatchInExpression(ast)
        ...
        if isinstance(ast.obj, Id):
            if "$" in ast.method.name:
                ...
                call_method = D96_utils.find_method(
                    ast.obj.name, ast.method.name, scope["global"], self.inheritance
                )
                if call_method is None:
                    raise Undeclared(Method(), ast.method.name)
                if call_method.kind != "method":
                    raise Undeclared(Method(), ast.method.name)
                if call_method.si_kind == "instance":
                    raise IllegalMemberAccess(ast)
                if type(call_method.type) == VoidType:
                    raise TypeMismatchInExpression(ast)
                # Check type param
        ...
        if type(obj_type) != ClassType:
            raise TypeMismatchInExpression(ast)
        call_method = D96_utils.find_method(
            obj_type.classname.name, ast.method.name, scope["global"], self.inheritance
        )
        if call_method is None:
            raise Undeclared(Method(), ast.method.name)
        if call_method.kind != "method":
        ...
        # Check type param
        argument_type = [self.visit(param, scope) for param in ast.param]
        if not D96_utils.check_param_type(call_method, argument_type, self.inheritance):
            raise TypeMismatchInExpression(ast)
        return call_method.type
    ...
    def visitAssign(self, ast, scope):
        ...
        if isinstance(lhs_type, D96_type):
            if lhs_type.kind == "immutable" or lhs_type.kind == "constant":
                raise CannotAssignToConstant(ast)
            lhs_type = lhs_type.type
        if isinstance(rhs_type, D96_type):
            rhs_type = rhs_type.type
        if not D96_utils.compare(lhs_type, rhs_type) and not D96_utils.coercion(
            rhs_type, lhs_type, self.inheritance
        ):
            raise TypeMismatchInStatement(ast)
    ...
    def visitIf(self, ast, scope):
        ...
        scope = scope[1]
        condition_type = self.visit(ast.expr, scope)
        if isinstance(condition_type, D96_type):
            condition_type = condition_type.type
        if type(condition_type) != BoolType:
            if self.top_level_if_statement:
                raise TypeMismatchInStatement(self.top_level_if_statement)
            raise TypeMismatchInStatement(ast)
        ...
        if isinstance(ast.elseStmt, If):
            ...
            if self.top_level_if_statement is None:
                self.top_level_if_statement = ast
        else:
            self.top_level_if_statement = None
        if ast.elseStmt:
    ...
    def visitFor(self, ast, scope):
        ...
        if id_type.kind == "constant" or id_type.kind == "immutable":
        ...
        if type(expr1_type) != IntType or type(expr2_type) != IntType:
            raise TypeMismatchInStatement(ast)
        if type(id_type) != IntType and type(id_type) != FloatType:
            raise TypeMismatchInStatement(ast)
        if ast.expr3:
            expr3_type = self.visit(ast.expr3, scope)
            if isinstance(expr3_type, D96_type):
                expr3_type = expr3_type.type
    ...
    def visitReturn(self, ast, scope):
        ...
        if (
            ...
            and self.current_class == "Program"
            and ast.expr is not None
        ):
            raise TypeMismatchInStatement(ast)
        if self.current_method == "Constructor" and ast.expr is not None:
            raise TypeMismatchInStatement(ast)
        if self.current_method == "Destructor":
            raise TypeMismatchInStatement(ast)
        ...
        # Suy diễn kiểu
        current_method = scope["global"][self.current_class].find_method(
            self.current_method
        )
        if current_method.type is None:
            ...
            if not D96_utils.compare(
                currnet_return_type, current_method.type
            ) and not D96_utils.coercion(
                currnet_return_type, current_method.type, self.inheritance
            ):
                raise TypeMismatchInStatement(ast)
