...
indexed_array     
    locals [size = 0]:   
    ARRAY LP array_list_of_expressions? RP;

array_list_of_expressions   :   expression{$indexed_array::size += 1} 
                                (COMMA expression {$indexed_array::size += 1})*;

multi_demensional_array     :   ARRAY LP array_literal_list? RP;
array_literal_list          :   array_literal
                                (COMMA array_literal)*; 
